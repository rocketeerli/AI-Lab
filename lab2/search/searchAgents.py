...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    heuristic = 0
    currentPos = state[0]             # 当前状态的位置
    unvisited = []
    for corner in corners :
        if not corner in state[1]:
            unvisited = unvisited + [corner]
    while not len(unvisited) == 0:                  
        distances = []
        for corner in unvisited:                     # 每次取出一个角落点
            distances += [(util.manhattanDistance(currentPos, corner), corner)]           # 当前点 和 角落的曼哈顿距离
        distance, corner = min(distances)            # 取出最小的角落距离
        heuristic = heuristic + distance
        currentPos = corner                          # 设置当前点为角落点，然后计算该角落点到下一个角落点的 曼哈顿距离
        unvisited.remove(corner)
    return heuristic                                 # 返回距离所有角落最短的 曼哈顿距离之和
...
def foodHeuristic(state, problem):
    ...
    result = 0
    for f in food:
        distance = mazeDistance(f, position, problem.startingGameState)  # 利用 bfs ， 查找迷宫中食物和当前位置的距离
        if result < distance:                # 选取最小的食物距离
            result = distance
    return result
